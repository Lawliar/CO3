...
def runCO3(args, debug = False, buggy_index = 98):
    ...
    if args.no_replace:
        backend_executable = os.path.join(CO3_DIR,"sym_backend","build_no_replace", "qsym_backend","orchestrator")
        spear_inter_dir    = os.path.join(CO3_DIR,"firmware","STMfirmware","Spear{}".format(benchmark),"intermediate_results_no_replace")
    elif args.no_app_shadow:
        backend_executable = os.path.join(CO3_DIR,"sym_backend","build_no_shadow", "qsym_backend","orchestrator")
        spear_inter_dir     = os.path.join(CO3_DIR,"firmware","STMfirmware","Spear{}".format(benchmark),"intermediate_results_no_shadow")
    else:
    ...
    while (True):
        for cur_input_id in range(batch_input_id_start, batch_input_id_end):
            ...
            cmd = [backend_executable,"-i",spear_inter_dir,"-p",str(args.p),"-b",str(args.baud)]
