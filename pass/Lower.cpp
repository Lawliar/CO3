...
LowerSwitch::switchConvert(CaseItr Begin, CaseItr End, ConstantInt *LowerBound,
  ...
  auto* Comp = CmpInst::Create(Instruction::ICmp,
                               ICmpInst::ICMP_SLT, Val,
                               Pivot.Low, "Pivot", NewNode);
...
{
  Function* F = OrigBlock->getParent();
  BasicBlock* NewLeaf = BasicBlock::Create(Val->getContext(), "LeafBlock", F);
...
unsigned LowerSwitch::Clusterify(CaseVector& Cases, SwitchInst *SI) {
  ...
  if (Cases.size() >= 2) {
    ...
    for (CaseItr J = std::next(I), E = Cases.end(); J != E; ++J) {
      if(J->Low->getBitWidth() > 64 || I->High->getBitWidth() > 64)
          continue;
...
static ConstantAggregate * hasConstExprInConstantAgg(Value *V){
    if(ConstantAggregate * cagg  = dyn_cast<ConstantAggregate>(V)){
...
void convertExpInAgg(ConstantAggregate * conAgg, Instruction * oldI,std::vector<Instruction *>* Worklist){
    //errs()<<"agg:"<<*conAgg<<"\n";
    Constant::op_iterator cagg_i = conAgg->op_begin();
    Constant::op_iterator cagg_e = conAgg->op_end();
    //std::vector<ConstantExpr*> toConvert;
    for( ;cagg_i != cagg_e ; cagg_i++){
        Value * operand = *cagg_i;
        if (ConstantExpr *CE = hasConstantExpr (operand)) {
            //toConvert.push_back(CE);
            Instruction *NewInst = convertExpression (CE, oldI);
            conAgg->replaceUsesOfWith (CE, NewInst);
    ...
    for (Function::iterator BB = (F).begin(); BB != (F).end(); ++BB) {
        for (BasicBlock::iterator i = BB->begin(); i != BB->end(); ++i) {
            ...
            for (unsigned index = 0; index < I->getNumOperands(); ++index) {
                ...
                else if(hasConstExprInConstantAgg(I->getOperand(index))){
                    Worklist.push_back(I);
                }
            }
        }
    ...
    while (Worklist.size()) {
        ...
        if (PHINode *PHI = dyn_cast<PHINode>(I)) {
            ...
            for (unsigned index = 0; index < I->getNumOperands(); ++index) {
                ...
                // constant expression.
                Value* oprand = I->getOperand(index);
                if (ConstantExpr *CE = hasConstantExpr (oprand)) {
                    Instruction *NewInst = convertExpression (CE, I);
                    I->replaceUsesOfWith (CE, NewInst);
                    Worklist.push_back (NewInst);
                }
                else if(ConstantAggregate * Cagg = hasConstExprInConstantAgg(oprand)){
                    convertExpInAgg(Cagg,I,&Worklist);
                }
            }
        }
    }
...
void breakConstantExpr(Function &F) {
