...
uint16_t adc_data[ADC_NUM_CHANNELS]  = {0};
...
uint16_t current_max_period          = 0;
uint32_t current_total               = 0;
float    current_total_mag_period    = 0;
uint32_t current_total_mag           = 0;
uint16_t voltage_max                 = 0;
uint16_t voltage_max_period          = 0;
uint32_t voltage_total               = 0;
uint16_t max_brightness              = 500;
...
static uint32_t get_current(void)
{
    ...
    else
    {
        ...
        if (adc_data[1] == 0)
        	...
        	return (1448 * 556) / current_total_mag_period;
    }
...
static uint32_t get_voltage(void)
{
    ...
    else
    {
        ...
        if (adc_data[0] == 0)
        	...
        	return (347800 * 15.5) / voltage_max_period;
    }
}
...
static uint32_t get_active_power(void)
{
    ...
    else
    {
        ...
        if ((adc_data[0] == 0) || (adc_data[1] == 0))
        	...
        	return (880373 * 15.5 * 556) / (voltage_max_period * current_total_mag_period);
    }
}
...
{
	dataRX = d; //hacky way to run the shelly dimmer
