...
RX_BUFF_TYPE cli_rx_buff     __attribute__( ( aligned( 128) ) ); /* 128byte FIFO, saving commands from the terminal */
...
static uint8_t cli_reboot(void *para, uint8_t len)
{
    ...
    //SYSTEM_REBOOT(); we don't want to reboot our system
...
HISTORY_S history   __attribute__( ( aligned(  next_power_of_2(sizeof( HISTORY_S )) ) ) );
...
void cli_init(uint32_t baud)
{
    ...
    //USART_INIT(baud); this is not necessary
...
HANDLE_TYPE_S Handle   __attribute__( ( aligned( 128) ) );
            ...
            if(TRUE == QUEUE_OUT((*rx_buff), Handle.buff[Handle.len])) {
                ...
                if ((0 == key) && (i < Handle.len)) {
                    ...
                    for (; i < Handle.len; i++) {
                    	...
                    	//HAL_UART_Transmit(&huart3, &Handle.buff[i], 1, 10);
                    }
#if CLI_HISTORY
                }
#endif  /* CLI_HISTORY */
