...
#symcc_tmp_input_dir      = "{}/tmp_in".format(symcc_output_dir)

spear_output_dir          = "{}/output".format(spear_inter_dir)
spear_tmp_output_dir      = "{}/tmp_out".format(spear_output_dir)
#spear_tmp_input_dir      = "{}/tmp_in".format(spear_output_dir)
...
assert(os.path.exists(symcc_output_dir))
...
def get_highest_id(input_dir):
    ...
    for each_input_file in os.listdir(input_dir):
        ...
        digits = int(each_input_file[:6],10)
...
def runSpear():
    ...
    shutil.copyfile(concrete_input, os.path.join(spear_output_dir,os.path.basename('0'.zfill(6))))
    ...
    for it in range(numIteration):
        ...
        for cur_spear_input_id in range(spear_input_cur_id, spear_output_cur_id):
            spear_input_file = os.path.join(spear_output_dir, str(cur_spear_input_id).zfill(6))
            ...
            p1.wait()
            print("iter:{}, cur:{}/{},total:{}".format(it, cur_spear_input_id - spear_input_cur_id , batch_size,cur_spear_input_id))
            ...
            for each_spear_output in os.listdir(spear_tmp_output_dir):
                ...
                dest_file_name = str(tmp_output_id).zfill(6)
                tmp_output_id += 1
...
def runSymcc():
    ...
    assert(os.path.exists(concrete_input))
    shutil.copyfile(concrete_input, os.path.join(symcc_output_dir,os.path.basename('0'.zfill(6))))
    ...
    for it in range(numIteration):
        ...
        for cur_symcc_input_id in range(symcc_input_cur_id, symcc_output_cur_id):
            symcc_input_file = os.path.join(symcc_output_dir, str(cur_symcc_input_id).zfill(6))
            ...
            p1.wait()
            print("iter:{},cur: {} / {},total:{}".format(it, cur_symcc_input_id - symcc_input_cur_id ,batch_size ,cur_symcc_input_id))
...
def main():
    #symcc_num = runSymcc()
    spear_num = runSpear()
    #print("symcc generate:{}\n".format(len(symcc_num)))
    print("spear generate:{}\n".format(len(spear_num)))
