...
static void targetTask( void * pvParameters )
{
   ...
   Fuzzer_t *pAFLfuzzer = (Fuzzer_t *)AFLfuzzerRegion;
    ...
    xTaskNotifyIndexed(AFLfuzzer.xTaskFuzzer,0,1,eSetValueWithOverwrite); //notify the fuzzer task the target is ready
    while(1)
    {
    	ulTaskNotifyTakeIndexed(0, pdTRUE, portMAX_DELAY); //wait for data coming from USART
